...
<html>
   ...
   <script>
       var timer=setInterval(function(){updatepage()},30000); //update 30 detik sekali
       function updatepage()
       {
           var xmlhttp;
           ...
           xmlhttp.open("GET","pagekomentar1.jsp?halaman=1&ID_tugas=<%out.print(ID_tugas);%>",true);
           xmlhttp.send();
       }
   </script>
    <body>
        ...
        <%
           ...
           rst=st.executeQuery("select avatar from user1 where username='"+arr_pembuat.get(i)+"';");
			...
			function hapuskomen<%out.print(i);%>()
			{
				...
				xmlhttp.onreadystatechange=function()
				{
					if (xmlhttp.readyState == 4 || xmlhttp.status == 200)
					{
						document.getElementById("komen<%out.print(i);%>").innerHTML="komentar terhapus";
					}		
				}
           ...
           // 10 5 
            ...
            if (sisa > 0) {out.println("<a href=\"pagekomentar.jsp?ID_tugas="+ID_tugas+"&halaman="+(halaman+1)+"\">Halaman berikutnya</a>");};
            if (halaman > 1) {out.println("<a href=\"pagekomentar.jsp?ID_tugas="+ID_tugas+"halaman="+(halaman-1)+"\">Halaman sebelumnya</a>");};
