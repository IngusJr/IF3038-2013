...
public class tugasList {
    ...
    public static void fill(String selectQuery) {
        ...
        try {
            ...
            for (int i = 0; i < aTugas.size(); i++) {
                tSet = MyDatabase.getSingleton().selectDB("SELECT * FROM asignee_tugas1 WHERE ID_tugas='"+aTugas.get(i).ID_tugas+"';");
                if(tSet.next()) aTugas.get(i).asignee=tSet.getString("username");
                while(tSet.next()) {
                    aTugas.get(i).asignee+=","+tSet.getString("username");
                }
            }
            for (int i = 0; i < aTugas.size(); i++) {
